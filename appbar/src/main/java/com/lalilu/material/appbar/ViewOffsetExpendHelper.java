...
class ViewOffsetExpendHelper {
    ...
    public ViewOffsetExpendHelper(View view) {
        this.view = view;
    }

    void onViewLayout() {
        // Grab the original top and left
        layoutBottom = view.getBottom();
        layoutTop = view.getTop();
        layoutLeft = view.getLeft();
    }

    void applyOffsets() {
        ViewCompat.offsetTopAndBottom(view, Math.min(offsetTop, 0) - (view.getTop() - layoutTop));
        ViewCompat.offsetLeftAndRight(view, offsetLeft - (view.getLeft() - layoutLeft));
        view.setBottom(layoutBottom + offsetTop);
    }

    /**
     * Set the top and bottom offset for this {@link ViewOffsetExpendHelper}'s view.
     *
    ...
    /**
     * Set the left and right offset for this {@link ViewOffsetExpendHelper}'s view.
     *
     * @param offset the offset in px.
     * @return true if the offset has changed
     */
